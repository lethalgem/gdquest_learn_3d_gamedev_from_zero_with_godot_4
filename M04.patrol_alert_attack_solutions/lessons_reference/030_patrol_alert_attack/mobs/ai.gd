...
#END:fsm_func_activate
	...
	func set_is_debugging(new_value: bool) -> void:
		...
		if current_state != null and current_state.mob != null and current_state.mob.debug_label != null:
...
## to the next state.
#ANCHOR:class_state_look_at_player
...
#END:class_state_look_at_player
...
class StateChase extends State:
	...
	func update(delta: float) -> Events:
		var player_position := Blackboard.player_global_position
		var desired_velocity := mob.global_position.direction_to(player_position) * chase_speed
		var velocity_distance := mob.velocity.distance_to(desired_velocity)
		mob.velocity = mob.velocity.move_toward(desired_velocity, velocity_distance * drag_factor * delta)
...
class StateCharge extends State:
	...
	## Time the mob spends anticipating the attack.
	var prepare_duration := 1.0
	...
	func enter() -> void:
		_duration = charge_distance / charge_speed
		_time = 0.0
